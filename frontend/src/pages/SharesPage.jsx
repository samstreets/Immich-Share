  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 2, height }}>
      {pts.map(({ day, view, upload }) => {
        ...
        return (
          <div key={day} title={`${day}: ${total} events`} style={{ flex: 1, height: `${h}%`, borderRadius: 2, background: total > 0 ? 'var(--accent)' : 'var(--border)', opacity: total > 0 ? 0.8 : 0.25, minHeight: 1 }} />
        )
      })}
    </div>
  )
...
function QRModal({ share, onClose }) {
  ...
  async function loadQR(dark) {
    setSvgContent(null); setError('')
    try {
      const dark_ = encodeURIComponent(dark ? '#1c2032' : '#13161f')
      const light_ = encodeURIComponent(dark ? '#ffffff' : '#ffffff')
  ...
  useEffect(() => { loadQR(darkMode) }, [share.id, darkMode])
      ...
      <div className="modal" style={{ maxWidth: 360 }}>
        ...
        <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
          <div style={{
            ...
            {svgContent
              ? <div dangerouslySetInnerHTML={{ __html: svgContent }} style={{ width: 280, height: 280 }} />
              : error
          ...
          <div style={{ fontSize: '0.75rem', color: 'var(--text-dim)', textAlign: 'center', fontFamily: 'monospace', wordBreak: 'break-all', padding: '0 10px' }}>
            {share.shareUrl}
          </div>
            ...
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 20 }}>
                {[
                  ...
                  <div key={label} style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', padding: '12px 14px' }}>
                    <div style={{ fontSize: '1.4rem', fontWeight: 800, color, letterSpacing: '-0.02em' }}>{value}</div>
                    <div style={{ fontSize: '0.72rem', color: 'var(--text-dim)', fontWeight: 600, marginTop: 3 }}>{label}</div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: '0.72rem', color: 'var(--text-dim)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 8 }}>
                    ...
                    <div key={action} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                      ...
                      <div style={{ flex: 1, height: 6, background: 'var(--bg3)', borderRadius: 999, overflow: 'hidden' }}>
                        <div style={{
                          ...
                          background: action === 'view' ? 'var(--accent)' : 'var(--green)',
                          borderRadius: 999,
              ...
              <>
                ...
                {form.share_type === 'album' && (
                  <div className="form-group">
                    ...
                    {albums.length === 0 && !sourceLoading && <span className="hint">No albums found — check your Immich connection in Settings.</span>}
                  </div>
                )}
                {form.share_type === 'tag' && (
                  <div className="form-group">
                    ...
                    {tags.length === 0 && !sourceLoading && <span className="hint">No tags found — check your Immich connection in Settings.</span>}
                  </div>
                )}
              </>
            )}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {selected.length} share{selected.length !== 1 ? 's' : ''} selected
        <button onClick={onSelectAll} className="btn btn-ghost btn-xs" style={{ color: 'var(--accent)', marginLeft: 4 }}>
          {selected.length === total ? 'Deselect all' : 'Select all'}
        </button>
        <button onClick={onClearAll} style={{ background: 'none', border: 'none', color: 'var(--text-dim)', cursor: 'pointer', fontSize: '0.8rem', padding: '0 4px' }}>✕</button>
      </div>
      <button className="btn btn-secondary btn-sm" onClick={() => onToggleSelected(true)}>
        ▶ Enable
      </button>
      <button className="btn btn-secondary btn-sm" onClick={() => onToggleSelected(false)}>
        ⏸ Disable
      </button>
      <button className="btn btn-danger btn-sm" onClick={onDeleteSelected} disabled={deleting}>
        {deleting ? <span className="loading-spinner" style={{ width: 12, height: 12 }} /> : '🗑 Delete Selected'}
      </button>
    </div>
  )
}
...
export default function SharesPage() {
  ...
  async function bulkDelete() {
    ...
    try {
      // Use bulk endpoint — fall back to individual if not available
      ...
      for (const id of selected) {
        try { await api(`/shares/${id}`, { method: 'DELETE' }) } catch {}
      }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <select value={typeFilter} onChange={e => handleFilter('type', e.target.value)} style={{ width: 'auto', minWidth: 110, flex: '0 0 auto' }}>
          <option value="">All types</option>
          <option value="album">Album</option>
          <option value="tag">Tag</option>
        </select>
        <select value={statusFilter} onChange={e => handleFilter('status', e.target.value)} style={{ width: 'auto', minWidth: 120, flex: '0 0 auto' }}>
        ...
        {(search || typeFilter || statusFilter) && (
          <button className="btn btn-ghost btn-sm" onClick={() => { setSearch(''); setTypeFilter(''); setStatusFilter(''); load({ search: '', type: '', status: '' }) }}>
            ✕ Clear
          </button>
        )}
        <span style={{ fontSize: '0.78rem', color: 'var(--text-dim)', marginLeft: 4 }}>
      ...
      {loading ? (
        ...
        <div style={{ background: 'var(--bg2)', border: '1.5px dashed var(--border)', borderRadius: 'var(--radius)', textAlign: 'center', padding: '70px 20px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, flexWrap: 'wrap' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 6, flexWrap: 'wrap' }}>
                      <span style={{ fontWeight: 800, fontSize: '0.95rem', letterSpacing: '-0.01em' }}>{share.name}</span>
                    ...
                    {share.description && (
                      <div style={{ fontSize: '0.83rem', color: 'var(--text-muted)', marginBottom: 7, fontWeight: 500 }}>
                        {share.description}
                      </div>
                    )}

                    {/* Meta strip */}
                    <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', fontSize: '0.75rem', color: 'var(--text-dim)', marginBottom: 10, fontWeight: 600 }}>
                      ...
                      {share.allow_download ? <span>⬇ Downloads on</span> : <span>⬇ Downloads off</span>}
                    </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                      <div style={{
                        ...
                        padding: '5px 12px', fontFamily: 'monospace', fontSize: '0.78rem', color: 'var(--accent)', fontWeight: 600,
                  ...
                  <div style={{ display: 'flex', gap: 5, flexShrink: 0, flexWrap: 'wrap', alignItems: 'flex-start' }}>
                    <button className="btn btn-secondary btn-sm" onClick={() => setQrShare(share)} title="Show QR code">
                      {/* QR icon */}
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        ...
                        <path d="M21 16h-3v3m3 0v2M16 16v5M16 13h5M3 10h2M7 10h2M10 10v2M10 7V3M13 3v2M13 7v2M10 13v2"/>
                      </svg>
                      QR
                    </button>
                    <button className="btn btn-secondary btn-sm" onClick={() => setStatsShare(share)} title="View stats">
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    ...
                    <button className="btn btn-secondary btn-sm" onClick={() => toggleActive(share)} title={share.is_active ? 'Disable' : 'Enable'}>
                      {share.is_active
      ...
      {/* Modals */}
      {showModal && (
        <ShareModal editShare={editShare} onClose={() => setShowModal(false)} onSaved={() => { setShowModal(false); load() }} />
      )}
